...
var ASM_CONSTS = {
  4727072: () => { Module['emscripten_get_now_backup'] = performance.now; },  
 4727127: ($0) => { performance.now = function() { return $0; }; },  
 4727175: ($0) => { performance.now = function() { return $0; }; },  
 4727223: () => { performance.now = Module['emscripten_get_now_backup']; }
};
...
// end include: preamble.js
  ...
  var es = {eventSourceInstances:{},nextInstanceId:1,Set:function(event) {
  		...
  		},_callOnError:function(errCallback, id, reason)
  		{
  			if (reason)
  			{
  				...
  				Module.dynCall_vi(errCallback, [id, 0]);
  ...
  function _UploadFile(gameObjectNamePtr, methodNamePtr, filterPtr, multiselect) {
          ...
          fileInput.onchange = function (event) {
              ...
              // File selected
              SendMessage(gameObjectName, methodName, JSON.stringify(fileInfos));
  ...
  function _XHR_GetResponseHeaders(request, callback)
  	{
  		...
  		getWasmTableEntry(callback)(request, buffer, byteArray.length);
  		...
  		if (http)
  		{
  			if (onprogress)
  				http.onprogress = function http_onprogress(e) {
  					...
  					if (e.lengthComputable)
  						getWasmTableEntry(onprogress)(request, e.loaded, e.total);
  			...
  			if (onuploadprogress)
  				http.upload.addEventListener("progress", function http_onprogress(e) {
  					...
  					if (e.lengthComputable)
  						getWasmTableEntry(onuploadprogress)(request, e.loaded, e.total);
  ...
  function _XHR_SetResponseHandler(request, onresponse, onerror, ontimeout, onaborted)
  	{
  		...
  		http.onload = function http_onload(e) {
  			...
  			if (onresponse)
  			{
  				...
  				getWasmTableEntry(onresponse)(request, http.status, 0, responseLength, 0);
